...
bool getJsonData(int invNum)
{
  ...
  liveData["BATT_WATTS"] = (int)(live.l.bV * batteryCurrent + 0.5)/ 100.f;
...
bool sendtoMQTT(int invNum)
{
  ...
  if (!_settings._mqttJson)
  {
    ...
    mqttclient.publish((topic + "/" + mqttDeviceName + "/LiveData/BATT_WATTS").c_str(), String((int)(live.l.bV * batteryCurrent +0.5)/ 100.f).c_str());
